...
main :: IO ()
main = do
    marcData <- B.readFile "inputFile.mrc"
...
allRecords marcStream =
    if marcStream == B.empty
        then []
        else record : allRecords restStream
    where (record, restStream) = nextAndRest marcStream
...
splitDirectory directory =
    if directory == B.empty
        then []
        else entry : splitDirectory restEntries
    where (entry, restEntries) = B.splitAt dirEntryLength directory
